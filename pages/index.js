...
const TIMINGS = {
  ...
  NAV_APPEAR: 6900,
  STICKERS_APPEAR: 7200, // NEW: Stickers appear after loading completes
};
...
export default function Home() {
  ...
  // NEW: Control when stickers are visible
  ...
  // ─── STICKER DATA (6 custom stickers) ───
  const stickers = [
    { 
      id: 1, 
      name: 'Hedie Shy', 
  ...
  useEffect(() => {
    if (userId && stickersVisible) { // Only fetch when stickers are visible
      fetchAllStickers();
      fetchUserStickers();
    }
  ...
  useEffect(() => {
    ...
    if (isRedirect === 'true') {
      ...
      setIsLoading(false);
      setIsReady(true);
      // Show stickers immediately for redirect
      ...
      setTimeout(() => {
        if (pageShellRef.current) {
          ...
          setTimeout(() => {
            ...
            setStickersVisible(true); // Show stickers after intro fade
  ...
  useEffect(() => {
    ...
    // NEW: Show stickers after loading completes
  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      ...
      if (isDragging && dragData) {
        ...
        const updatedStickers = placedStickers.map(s => {
          if (s.id === dragData.stickerId) {
        ...
        const updatedAllStickers = allStickers.map(s => {
  ...
  }, [bulgeEnabled, isPlacingSticker, selectedSticker, isDragging, dragData, placedStickers, allStickers]);
        ...
        try {
          const res = await fetch('/api/stickers/update', {
            ...
            body: JSON.stringify({
              ...
              userId: userId,
              x: dragData.originalX + (mouseRef.current.x - dragData.startX),
              y: dragData.originalY + (mouseRef.current.y - dragData.startY),
            }),
          });
